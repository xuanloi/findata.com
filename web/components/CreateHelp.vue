  ...
  <div class="media-right">
    ...
    <a class="image is-128x128" @click="gallery=image">
      ...
      <p class="has-text-centered">
        ...
        <a class="button is-primary is-light" @click="media = {name: 'help-image', open: true}"> Hình ảnh (nếu có) </a>
        </div>
  </div>
</article>
...
<div class="field is-horizontal mt30">
  ...
  <div class="field-body">
    <div class="field">
      <div class="control">
        <button class="button is-primary" @click="createHelp()">
          {{(pagehelp.record? pagehelp.record.id : false)? 'Cập nhật bài viết' : 'Tạo bài viết'}}
        </button>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    ...
    var connlist = this.connection.checkDataReady(['helpcategory'])
    if(connlist.find(v=>v.name==='helpcategory').ready) { this.topics = this.category.filter(v=>v.level===0)}
    let filter = connlist.filter(v=>!v.ready)
    if(filter.length>0) this.connection.getApi(filter)
    else if(this.pagehelp) this.fillData() 
  },
  ...
  watch: {
    ...
    media: function(newVal) {
      ...
      if(newVal.name==='help-image') this.image = newVal.select.file
    },
    ...
    'connection1.getupdateStatus': function(newVal) {
      if(newVal) {
        ...
        let copy = this.$copy(this.pagehelp.data)
        let index = copy.findIndex(v=>v.id===row.id)
        if(index>=0) copy[index] = row
        else copy.push(row)
        this.$store.commit('updateState', {name: 'pagehelp', key: 'data',  data: copy})
        this.$store.commit('updateState', {name: 'pagehelp', key: 'record',  data: row})
        this.fillData()      
      }
    }
  },
  ...
  methods: {
    fillData() {
      if(this.pagehelp.record? !this.pagehelp.record.id : false) {
      ...
      let help =  this.pagehelp.record
      this.title = help.title
      this.subtitle = help.subtitle
      this.content = help.content
      let category = this.category.find(v=>v.id===help.category)
      if(category.level===1) {
      ...
      this.creator = help.creator__name
      this.image = help.image
      this.creator_name = help.creator__name
      this.valid_from = help.valid_from? new Date(help.valid_from) : undefined
      this.valid_to = help.valid_to? new Date(help.valid_to) : undefined
      this.selectApprove = this.approvestatus.find(v=>parseInt(v.id)===help.approve_status)
      this.approve = this.selectApprove.value
      this.note = help.note
      this.approver  = help.approver__name
    },
    ...
    createHelp() {
      ...
      let id = this.pagehelp? (this.pagehelp.record? this.pagehelp.record.id : undefined) : undefined
      ...
      creator: id? this.pagehelp.record.creator : this.login.id, image: this.image? this.image : 'no-image.jpg', approve_status: this.selectApprove.id, 
      valid_from: this.valid_from? new Date(this.valid_from) : undefined,
      valid_to: this.valid_to? new Date(this.valid_to) : undefined, 
      approver: this.pagehelp.record.approver? this.pagehelp.record.approver : (this.selectApprove.code==='yes'? this.login.id : undefined) }

      if(!data.id) this.connection1.insert('help', data, this.pagehelp.api.params.values)
      else this.connection1.update('help', data, this.pagehelp.api.params.values)
    }
  }
}
