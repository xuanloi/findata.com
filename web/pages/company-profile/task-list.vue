...
export default {
    ...
    created() {
      ...
      let field = this.$createField('company__stock_code', 'Công ty', 'string', true, true)
      field.tooltip = {position: 'is-right', field: 'company__name', class: 'tag is-rounded is-light'}
      fields.push(field)

      fields.push(this.$createField('company__type__code', 'Loại', 'string', true, true))
      fields.push(this.$createField('report_period__value', 'Kỳ BC', 'string', true, true))
      fields.push(this.$createField('year', 'Năm', 'string', true, true))
      fields.push(this.$createField('report_name__code', 'Báo cáo', 'string', true, true))

      fields.push(this.$createField('assigner__name', 'Người giao việc', 'string', true, true))
      fields.push(this.$createField('recipient__name', 'Người nhận việc', 'string', true, true))

      fields.push(this.$createField('assign_date', 'Ngày giao', 'string', true, true))
      ...
      fields.push(this.$createField(name, text, 'string', true, true))
      fields.push(this.$createField('priority', 'Ưu tiên', 'string', true, true))
      
      field = this.$createField('status__value', 'Trạng thái', 'string', true, true)
      ...
      field = this.$createField('action', '', 'string', false, true, '7%', isAllow? 'select, edit' : 'select')
      fields.push(field)
      this.fields = fields
      ...
      'pagetask.record': function(newVal) {
        if(newVal) {
          if(this.pagetask.action==='edit') {
            let routeData = this.$router.resolve({ path: '/company-profile/task-edit', query: {task: newVal.id}})
             window.open(routeData.href, '_blank')
          } 
          else if(this.pagetask.action==='show') {
            let routeData = this.$router.resolve({ path: '/company-profile/data-entry', query: {task: newVal.id, type: newVal.company__type__code, company: newVal.company, stock_code: newVal.company__stock_code, report: newVal.report_name__code}})
             window.open(routeData.href, '_blank')
          } 
    ...
    methods: {
      ...
      connectData() {
        ...
        values += 'assigner,assigner__name,recipient,recipient__name,assign_date,due_date,priority,status,status__value,status__code,entry_time,waiting1_time,waiting2_time,approve_time,update_time'
        let nextdate = new Date(new Date(this.tdate).getTime()+(1*24*60*60*1000))
        
        let found = {name: 'tasklist', url: 'data/Task_Profile/', params: {}}
      ...
      fillData() {
        ...
        data.map(v=>{
          ...
          v.entry_time = mixing.isEmpty(v.entry_time)===true? 'n/a' : mixing.yyyymmdd(new Date(v.entry_time))
          v.waiting1_time = mixing.isEmpty(v.waiting1_time)===true? 'n/a' : mixing.yyyymmdd(new Date(v.waiting1_time))
          v.waiting2_time = mixing.isEmpty(v.waiting2_time)===true? 'n/a' : mixing.yyyymmdd(new Date(v.waiting2_time))
          v.approve_time = mixing.isEmpty(v.approve_time)===false? mixing.yyyymmdd(new Date(v.approve_time)) : undefined
        })
      ...
      getHeader() {
          ...
          if(status===undefined) this.tophead = 'Danh sách nhập Hồ sơ công ty'
      ...
      exportData() {
        ...
        let fields = ['id', 'company__stock_code', 'company__type__code', 'report_period__value', 'year', 'report_name__code', 
        'assigner__name', 'recipient__name', 'assign_date', 'due_date','priority', 'status__value', 'report', 'approve', 'entry_time', 'waiting1_time', 'waiting2_time', 'approve_time']
        this.$exportExcel(this.pagetask.filter, this.tophead, fields, dataTypes)
      }
    }
}
</script>