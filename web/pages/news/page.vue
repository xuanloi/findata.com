...
<template>
<div>
    <TopMenu v-bind="{type: 'tophead', tophead: 'Chào mừng ' + (login? login.name : 'bạn') + ' đến với trang thông báo'}" />
  ...
  <div class="tile is-vertical is-9">
    ...
    <div class="tile is-parent is-7">
      ...
      <div class="tile is-parent">
        <article class="tile is-child">
        <div class="ml15 mr15">
      <nuxt-link class="fs24 fb500 has-text-dark" :to="{path: '/news/open', query: {id: headline[0].id}}">
        {{headline[0].title}} 
      </nuxt-link>
      <p class="mt10 fs16 has-text-dark content" v-for="(v,i) in headline.slice(0,1)" :key="i"> 
          ...
          :to="{path: '/news/category', query: {id: v.category}}"> {{v.category__value}} </nuxt-link>
       </p>
        </div>
        </article>
      </div>
    ...
    <div class="tile is-parent">
      ...
      <nuxt-link class="fs18 fb500 has-text-dark" :to="{path: '/news/open', query: {id: v.id}}">
        {{v.title}} 
      </nuxt-link>
        <p class="mt10 fs16 has-text-dark mb20 content">  {{v.subtitle}} 
          <nuxt-link class="tag has-background-white tagborder is-rounded"
          :to="{path: '/news/category', query: {id: v.category}}"> {{v.category__value}} 
          </nuxt-link> 
        </p>
        </div>
      </article>
  ...
  <div v-for="(v,i) in topic" :key="i">
    ...
    <div class="level-left">
        ...
        <p class="title"> 
          <nuxt-link class="has-text-dark" :to="{path: '/news/category', query: {id: v.id}}"> {{v.value}} </nuxt-link>
          </p>
        </div>
          <div class="level-item">
              <p class="tags">
        <nuxt-link class="tag is-medium is-rounded"  :to="{path: '/news/category', query: {id: x.id}}"
    ...
    <div class="level-right" v-if="v.data.length>0">
        <p class="level-item">
        <nuxt-link class="fs16" :to="{path: '/news/category', query: {id: v.id}}"> Xem thêm 
    ...
    <div class="tile is-ancestor" v-else>
      ...
      <nuxt-link  :to="{path: '/news/open', query: {id: x.id}}" v-if="!x.dummy">
      <img v-lazy="connection.path + 'static/images/' + x.image">
              ...
              <article class="tile is-child">
                <p class="ml20 fs18 fb500" v-if="!x.dummy"> 
                 <nuxt-link class="has-text-dark" :to="{path: '/news/open', query: {id: x.id}}"> {{x.title}} </nuxt-link>
                 </p>
              </article>
          </div>
        </div>
      ...
      <nuxt-link class="fs18 fb500 has-text-dark" :to="{path: '/news/open', query: {id: x.id}}" v-if="!x.dummy">
        {{x.title}} 
      </nuxt-link>
        <p class="mt10 fs16 has-text-dark content" v-if="!x.dummy">  {{x.subtitle}} 
          <nuxt-link class="tag has-background-white tagborder is-rounded"
          :to="{path: '/news/category', query: {id: x.category}}"> {{x.category__value}} 
          </nuxt-link> 
        </p>
        </div>
      </article>
        ...
        <nuxt-link class="image" :to="{path: '/news/open', query: {id: x.id}}">
        <img v-lazy="connection.path + 'static/images/' + x.image">
        </nuxt-link>

        <div class="mt10"> 
          <nuxt-link class="fs18 fb500 has-text-dark" :to="{path: '/news/open', query: {id: x.id}}"> {{x.name}}
              {{x.title}} 
          </nuxt-link>
          <p class="mt10 fs16 has-text-dark content">  {{x.subtitle}} 
           <nuxt-link class="tag has-background-white tagborder is-rounded"
           :to="{path: '/news/category', query: {id: x.category}}"> {{x.category__value}} </nuxt-link> 
          </p>
        </div>
            </div>
        </article>
...
export default {
  ...
  watch: {
    ...
    'connection1.getdataReady': function(newVal) {
      if (newVal==='success') {
        this.newslist = this.connection1.getbatchData
        this.fillData()
      }
    }
  },
  ...
  created() {
    let list = this.connection.checkDataReady(['category'])
  ...
  computed: {
    ...
    newslist: {
      get: function() {return this.$store.state.newslist},
      set: function(val) {this.$store.commit('updateNewsList', {newslist: val})}
    },
  },
  ...
  methods: {
    getData() {
      ...
      this.category.map(v=>{
        let val = {name: v.id, url: 'data/News', params: {values: values, page: 1, perpage: 10, filter: {category: v.id}}}
        connlist.push(val)
      })
    ...
    fillData() {
      ...
      this.topic.map(v=>{
        ...
        for (let index = 0; index < 6; index++) {
          if(v.data.length<6) {
            list.map(x=>{
              let found = this.newslist.find(y=>y.name===x.id)
